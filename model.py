...
class Discriminator(nn.Module):
    ...
    def __conv_block(self, inchannel, outchannel, kernel_size, stride, padding, layer=None):
        if layer == "first_layer":  # 第一层没有BN
        ...
        elif layer == "last_layer":  # 最后一层没有BN
            conv = nn.Conv2d(inchannel, outchannel, kernel_size, stride, padding, bias=False)
            sigmoid = nn.Sigmoid()
            ...
            leakrelu = nn.LeakyReLU(0.2, inplace=True)  # ppt中设定负斜率为0.2
...
def weight_init(m):  # 权重初始化
    ...
    elif classname.find('BatchNorm') != -1:# BN层
